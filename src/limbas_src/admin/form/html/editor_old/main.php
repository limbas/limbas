...
<TR id="menu_multi_js_php_text_chart_templ_datum_relpath_notice_filter_globsearch_dbdat_dbdesc_dbnew_dbsearch_submt_button_inptext_inphidden_inparea_inpselect_inpcheck_inpradio_frame_uform_tile_" STYLE="display:none"><TD>
        <div class="lmbContextRow">
            <div class="lmbContextItem" style="text-align:right">
                ...
                <select class="contextmenu" name="input_fontface" id="input_fontface" size="1" onchange="fill_style('0','fontFamily',this.value);" style="vertical-align:middle;width:94px;" autocomplete="off">
                    <option value=""></option>
...
function printFormularElement($form_ID,$elementType,$printParams,$closediv=null,$elementExtension=null,$extendetStyle=null,$tabCell=null)
{
	...
	if($elementType == 'bild')
	{
        $form_picname = $printParams["TAB_SIZE"];
		$size1 = "width=\"".$printParams["WIDTH"]."\" height=\"".$printParams["HEIGHT"]."\"";
		if($form_convert){
		$path = TEMPPATH . 'thumpnails/form/';
		}else{
			$path = UPLOADPATH . 'form/';
		}

        // thumbnail
        $path = lmb_getDownloadHash($path,$form_picname,$GLOBALS['umgvar']["thumbnail_type"]);
	...
	if($elementType == 'php' || $elementType == 'uform' || $elementType == 'dbsearch' || $elementType == 'dbnew' || $elementType == 'dbdesc' || $elementType == 'dbdat' || $elementType == 'bild' || $elementType == 'usetime' || $elementType == 'inpradio' || $elementType == 'text' || $elementType == 'tabuItem' || $elementType == "js" || $elementType == "rect" || $elementType == "line" || $elementType == "ellipse" || $elementType == "datum" || $elementType == "relpath" || $elementType == "notice"  || $elementType == "filter"|| $elementType == "globsearch"||  $elementType == "inptext" || $elementType == "inphidden" || $elementType == "inparea" || $elementType == "inpselect" || $elementType == "inpcheck" || $elementType == "submt" || $elementType == "button" || $elementType == "tabcell")
	{
		...
		$params = "$params,'EVENT1','".htmljs(urlencode($printParams["EVENT"][0]))."'";
		$params = "$params,'EVENT2','".htmljs(urlencode($printParams["EVENT"][1]))."'";
		$params = "$params,'EVENT3','".htmljs(urlencode($printParams["EVENT"][2]))."'";
		$params = "$params,'EVENT4','".htmljs(urlencode($printParams["EVENT"][3]))."'";
		$params = "$params,'EVENT5','".htmljs(urlencode($printParams["EVENT"][4]))."'";
	...
	foreach ($form["id"] as $key => $value){
		...
		if($form["typ"][$key] == "menue"){
			...
			if($gfield[$referenz_tab]["sort"]){
                ...
                if(in_array(100,$gfield[$referenz_tab]["field_type"])){
