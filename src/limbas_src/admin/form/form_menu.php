   ...
   if ($alert) {
    ...
    function resetmenu() {
        ...
        if ($form["form_typ"] == 1 || $form["form_typ"] == 2) {
            array_push($reset, 'wflhist', 'reminder', 'dbdat', 'dbdesc', 'dbnew', 'text', 'datum', 'relpath', 'notice','filter','globsearch','scroll', 'js', 'php', 'submt', 'button', 'inptext', 'inparea', 'inpselect', 'inpcheck', 'inpradio', 'inphidden', 'chart', 'templ', 'frame','tabulator','tab','uform','html');
...
<TABLE BORDER="0" cellspacing="0" cellpadding="0">
    <TR>
        ...
        <FORM ENCTYPE="multipart/form-data" ACTION="main_admin.php" METHOD="post" name="form1">
            ...
            <input type="hidden" name="default_header">
                ...
                <TABLE cellspacing="0" cellpadding="2" class="formeditorPanel">
                    ...
                    <TR>
                        ...
                        <TD COLSPAN="2" align="right">
                            <select OnChange="document.form1.default_class.value = this.value; document.form1.submit();" style="width:110px;">
                                <option value="NULL">
                            ...
                            <TABLE BORDER="0" cellspacing="0" cellpadding="0">
                                ...
                                <TR>
                                 ...
                                 <TR>
                                    ...
                                    <?php if ($form["form_typ"] == 2) {$st = "";} else {$st = "none";} ?>
                                    <TD VALIGN="TOP" STYLE="display:<?= $st ?>;"><i ID="filter" class="lmb-icon lmb-eye btn" TITLE="filter" VALUE="filter" OnMouseDown="pressbutton('filter', 'inset', '<?= $farbschema['WEB10'] ?>');" OnMouseUp="pressbutton('filter', 'outset', '<?= $farbschema['WEB7'] ?>');add('filter');send();"></i></TD>
                                    <?php if ($form["form_typ"] == 2) {$st = "";} else {$st = "none";} ?>
                                    <TD VALIGN="TOP" STYLE="display:<?= $st ?>;"><i ID="globsearch" class="lmb-icon lmb-rep-dbsearch btn" TITLE="global search" VALUE="globsearch" OnMouseDown="pressbutton('globsearch', 'inset', '<?= $farbschema['WEB10'] ?>');" OnMouseUp="pressbutton('globsearch', 'outset', '<?= $farbschema['WEB7'] ?>');add('globsearch');send();"></i></TD>
                                 </TR>
                            </TABLE>
                        </TD>
                    </TR>
                            ...
                            <td>
                               ...
                               $gtabid = $form["referenz_tab"];
                                if ($form["form_typ"] != 3 and $gfield[$gtabid]["id"]) {
                                    require_once(COREPATH . 'admin/form/form_tabliste.php');
                                }
                                ?>
                            </td>
                        </tr>
