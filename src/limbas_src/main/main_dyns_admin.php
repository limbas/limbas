...
function dyns_fileGroupRules($para){
	...
	if(is_numeric($para["level"])){
		...
		echo "<table class=\"table table-sm p-0 m-0 table-borderless\">";
		echo "<tr>
...
function dyns_reportTabFieldList($para){
	...
	foreach ($gfield[$gtabid]["sort"] as $key => $value){
		...
		if($gfield[$gtabid]["field_type"][$key] == 11 AND $gfield[$gtabid]["verkntabid"][$key]){
			...
			echo "<TR><TD><IMG class=\"lmb-image-as-icon\" SRC=\"assets/images/legacy/outliner/".$outliner.".gif\" WIDTH=\"18\" HEIGHT=\"16\" ALIGN=\"TOP\" BORDER=\"0\">&nbsp;<A onclick=\"add_dbfield(this,event);\" title=\"".$gfield[$gtabid]["field_name"][$key]."\" lmfieldid=\"$key\" lmgtabid=\"$gtabid\" lmparentrel=\"$parentrel\" lmdatatype=\"".$gfield[$gtabid]["data_type"][$key]."\">".$gfield[$gtabid]["spelling"][$key]."</A></TD></TR>\n";
		}
	...
	foreach ($gfield[$gtabid]["sort"] as $key => $value){
		...
		if($gfield[$gtabid]["field_type"][$key] == 11 AND $gfield[$gtabid]["verkntabid"][$key]){
			...
			foreach ($gtab["raverkn"][$gtab["verkn"][$gfield[$gtabid]["verkntabid"][$key]]] as $keyskn => $valueskn){
				...
				echo "<TR><TD>\n";
				echo "<TABLE BORDER=\"0\" CELLPADDING=\"0\" CELLSPACING=\"0\"><TR><TD class=\"lmb-image-as-icon\" BACKGROUND=\"assets/images/legacy/outliner/line.gif\" VALIGN=\"top\"><IMG SRC=\"assets/images/legacy/outliner/".$outliner.".gif\" WIDTH=\"18\" HEIGHT=\"16\" ALIGN=\"TOP\" BORDER=\"0\"></TD><TD>\n";

				echo "<TABLE BORDER=\"0\" CELLPADDING=\"0\" CELLSPACING=\"0\">\n";
				echo "<TR><TD TITLE=\"table : ".$gtab["desc"][$valueskn]."\"><A HREF=\"Javascript:LmAdm_getFields(".$valueskn.",$globid,$gtabid,$key,'".$parentrelpath."|".$parentrelpath2."')\"><IMG class=\"lmb-image-as-icon\" SRC=\"assets/images/legacy/outliner/plusonly.gif\" WIDTH=\"18\" HEIGHT=\"16\" ALIGN=\"TOP\" BORDER=\"0\" NAME=\"tab_".$globid."_plusminus\"><i class=\"lmb-icon lmb-folder-closed\" WIDTH=\"16\" HEIGHT=\"13\" ALIGN=\"TOP\" BORDER=\"0\" NAME=\"tab_".$globid."_box\"></i></A> <B title=\"".$gtab["table"][$keyskn]."\">".$gtab["desc"][$keyskn]."</B></TD></TR>\n";
				echo "<TR><TD ID=\"el_$globid\">";
		...
		echo "<TR><TD><IMG class=\"lmb-image-as-icon\" SRC=\"assets/images/legacy/outliner/".$outliner.".gif\" WIDTH=\"18\" HEIGHT=\"16\" ALIGN=\"TOP\" BORDER=\"0\">";
		if($gfield[$gtabid]["field_type"][$key] == 11 AND $gfield[$gtabid]["verkntabid"][$key]){echo "<IMG class=\"lmb-image-as-icon\" SRC=\"assets/images/legacy/outliner/hline.gif\" WIDTH=\"18\" HEIGHT=\"16\" ALIGN=\"TOP\" BORDER=\"0\">";}
    ...
    if ($r_verkntabid = $gfield[$gtabid]["r_verkntabid"]) {
        foreach ($r_verkntabid as $key_ => $gtabid_) {
            ...
            if($verknparams = $gfield[$gtabid_]["verknparams"][$r_verknfieldid]){
                ...
                echo "<TR><TD>\n";
                echo "<TABLE BORDER=\"0\" CELLPADDING=\"0\" CELLSPACING=\"0\"><TR><TD class=\"lmb-image-as-icon\"  BACKGROUND=\"assets/images/legacy/outliner/line.gif\" VALIGN=\"top\"><IMG SRC=\"assets/images/legacy/outliner/join.gif\" WIDTH=\"18\" HEIGHT=\"16\" ALIGN=\"TOP\" BORDER=\"0\"></TD><TD>\n";
                
                echo "<TABLE BORDER=\"0\" CELLPADDING=\"0\" CELLSPACING=\"0\">\n";
                echo "<TR><TD TITLE=\"table : " . $gtab["desc"][$gtabid_] . "\"><A HREF=\"Javascript:LmAdm_getFields(" . $verknparams . ",$globid,$gtabid_,$r_verknfieldid,'')\"><IMG class=\"lmb-image-as-icon\" SRC=\"assets/images/legacy/outliner/plusonly.gif\" WIDTH=\"18\" HEIGHT=\"16\" ALIGN=\"TOP\" BORDER=\"0\" NAME=\"tab_" . $globid . "_plusminus\"><i class=\"lmb-icon lmb-folder-closed\" WIDTH=\"16\" HEIGHT=\"13\" ALIGN=\"TOP\" BORDER=\"0\" NAME=\"tab_" . $globid . "_box\"></i></A> <B title=\"field: ".$gfield[$gtabid_]["spelling"][$r_verknfieldid]."\"><i>".$gtab["desc"][$verknparams]."</i></B></TD></TR>\n";
                echo "<TR><TD ID=\"el_$globid\">";
...
function dyns_VieweditorFields($par){
	...
	if(isset($par["setviewfield"])){
		lmb_editQuestFields($viewid,$par);
	}
...
function dyns_createDiagPChart($par){
	if($path = LmbChart::makeChart($par['diag_id'])){
        echo lmb_getDownloadHash($path,'','image/png');
	}
}
...
/**
 * userComparePermission
 *
 * @param unknown_type $par
 */
function dyns_userComparePermission($par){
	$user_id = intval($par['user_id']);
	$compare_id = intval($par['compare_id']);
    if($compare_id) {
        require_once(COREPATH . 'admin/user/user_comparePermission.php');
    }else{
        require_once(COREPATH . 'admin/user/user_accumulatePermission.php');
    }
}
...
function dyns_syncValidate($par){
    ...
    $filter['filter_count'] = $par['filter_count'];
    $filter['filter_checksum'] = $par['filter_checksum'];
    $filter['filter_to'] = $par['filter_to'];
    $filter['filter_from'] = $par['filter_from'];
    ...
    if($par['phase'] == 2 && $par['rebuild']){
        ...
        echo json_encode(lmb_SyncValidatePhase3($par['syncid'],$filter));
    }else if ($par['phase'] == 4){
...
function dyns_manageDataSync($params): void
{
    global $alert;
    $dynsController = new DataSyncController();
    $result = $dynsController->handleRequest($params);
    $alert = null;
    echo json_encode($result);
}

function dyns_manageForms($params): void
{
    global $alert;
    $dynsController = new FormController();
    $result = $dynsController->handleRequest($params);
    $alert = null;
...
function dyns_manageUmgVars($params): void
{
    global $alert;
    $controller = new UmgVarController();
