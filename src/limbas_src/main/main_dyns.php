...
function dyns_14_b($params){
    ...
    if($searchvalue != "*"){
        ...
        foreach($gfield[$gtabid]["verknsearch"][$fieldid] as $vkey => $vval){
            ...
            $gsr_ = array();
            $gsr_[$vgtabid][$vval][0] = $searchvalue;
...
function dyns_fileSearch($params){
    ...
    $name_field = array_search('NAME',$gfield[$gtabid]['field_name']);
    $level_field = array_search('LEVEL',$gfield[$gtabid]['field_name']);
    $gsr[$gtabid][$name_field][0] = $searchvalue;
    $onlyfield[$gtabid] = array($name_field,5);
    $gresult = get_gresult($gtabid,1,$filter,$gsr,null,$onlyfield);

    $result = [];
    if($gresult[$gtabid]['id'][0]){
        /* ------------------ Default --------------------- */
        foreach ($gresult[$gtabid]['id'] as $key => $value) {
            $level = $gresult[$gtabid][$level_field][$key];
            $result[] = [
                'id' => $value,
                'level' => $level,
...
function dyns_getDiag($params){
    ...
    if ($gdiaglist[$gtabid]["id"][$diag_id]) {
        if ($gdiaglist[$gtabid]["template"][$diag_id] and file_exists($umgvar["pfad"] . $gdiaglist[$gtabid]["template"][$diag_id])) {
            ...
            if ($path = LmbChart::makeChart($diag_id, $gsr, $filter, null, null, null, null, [], $chartjs)) {
                echo trim($path);
            }
        }
    }
...
function dyns_versionDiff($params){
    ...
    if($out = dyns_versionDiffResult($params)){
        echo <<<HTML
            <table class="w-100">
                <tr><td nowrap><b>V{$v1}</b> $d1 </td><td nowrap class="text-end"><i>$u1</i></td></tr>
                <tr><td nowrap><b>V{$v2}</b> $d2 </td><td nowrap class="text-end"><i>$u2</i></td></tr>
            </table>
            <hr>
        HTML;
    ...
    if($gtab["raverkn"][$gtab["verkn"][$gtabid]]){
        ...
        foreach ($gtab["raverkn"][$gtab["verkn"][$gtabid]] as $key0 => $tabid){
            ...
            if($gtab["tab_id"][$tabid]){
                ...
                foreach ($gfield[$tabid]["sort"] as $key => $value){
                    ...
                    if(function_exists($fname) AND $gfield[$tabid]["data_type"][$key] != 22 AND $gfield[$tabid]["parse_type"][$key] < 100 AND $gfield[$tabid]["copy"][$key]){
                        ...
                        if($gfield[$tabid]["data_type"][$key] == 30){
                            if(!(trim($result1['value'])/1)){$result1 = "";}
                            if(!(trim($result2['value'])/1)){$result2 = "";}
                        }
                        ...
                        if(is_array($result1) OR is_array($result2)){
                            ...
                            if($diff){
                                ...
                                if(is_array($diff) AND lmb_count($diff) > 0){
                                    $out .= "<TR><TD>&nbsp;&nbsp;".$gfield[$tabid]["spelling"][$key]."</TD><TD>&nbsp;</TD><TD style=\"border:3px solid #CCCCCC\" TITLE=\"".$chuser."\">";
                                    ...
                                    $out .= "<TR><TD>&nbsp;&nbsp;".$gfield[$tabid]["spelling"][$key]."</TD><TD>&nbsp;</TD><TD TITLE=\"".$chuser."\">".html_entity_decode($diff,ENT_QUOTES,$GLOBALS["umgvar"]["charset"])."</TD></TR>";
                                }
                                $diff = null;
                            }
                ...
                if($out){
                    if(!$sub){$outd = "<TR><TD><b>".$gtab["desc"][$tabid]."</b></TD><TD COLSPAN=\"3\">";}
                    else{$outd = "<TR><TD COLSPAN=\"5\">";}
                    $out = $outd.$fieldkey.$fielddesc."</TD></TR>".$out;
                    $sumout[] = $out;
                }
        ...
        if(lmbdb_fetch_row($rs)){
            ...
            if(file_exists($target)){
                ...
                if(!lmb_utf8_check(lmb_substr($target2,0,200))){
                    $target2 = lmb_utf8_encode($target2);
                }
            }
        }
 ...
 * todo remove this, use handleReminder instead
 *
 * @param unknown_type $params
 */
function dyns_showReminder($params){
    ...
    $params['action'] = 'show';
    dyns_handleReminder($params);
}

function dyns_handleReminder($params): void
{
    $reminderController = new ReminderController();
    echo json_encode(
        $reminderController->handleRequest($params)
    );
}
...
function dyns_gtabFieldsSelect2($params) {
    ...
    foreach ($gfield[$tabID]['sort'] as $key => $fieldName) {
        ...
        if($gfield[$tabID]["detailsearch"][$key]){continue;}
