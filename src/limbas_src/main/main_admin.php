...
elseif ($action == 'setup_datasync' AND $LINK[$action] == 1) {
    $controllerAction = [DataSyncController::class, 'index', []];
    $BODYHEADER = $lang[$LINK['desc'][$LINK_ID[$action]]];
}
# AND $LINK[$action] == 1
...
elseif ($action == "setup_umgvar" AND $LINK[$action] == 1) {
    $controllerAction = [UmgVarController::class, 'index', []];
	$BODYHEADER = $lang[$LINK["desc"][$LINK_ID[$action]]];
}
...
elseif ($action == "setup_fonts" AND $LINK[$action] == 1) {
    $controllerAction = [FontController::class, 'index', []];
	$BODYHEADER = $lang[$LINK["desc"][$LINK_ID[$action]]];
}
...
elseif ($action == "setup_form_frameset" AND $LINK["setup_form"] == 1) {
    $controllerAction = [FormController::class, 'showOld', []];
}
elseif ($action == "setup_form_main" AND $LINK["setup_form"] == 1) {
	#$form_typ = 1;
	$require1 = "extra/explorer/filestructure.lib";
	$require2 = "admin/form/form.dao";
    $require3 = "admin/form/html/editor_old/main.php";
...
elseif ($action == "setup_form_menu" AND $LINK["setup_form"] == 1) {
    ...
    $require4 = "admin/form/html/editor_old/sidebar.php";
}
elseif ($action == 'setup_form_select' AND $LINK['setup_form'] == 1) {
	$action = 'setup_form';
    $controllerAction = [FormController::class, 'index', []];
    $BODYHEADER = $lang[$LINK['desc'][$LINK_ID[$action]]];
}
elseif ($action == 'setup_form_show' AND $LINK['setup_form'] == 1) {
    $action = 'setup_form';
    $controllerAction = [FormController::class, 'show', [$_GET['id'] ?? 0]];
    $BODYHEADER = $lang[$LINK['desc'][$LINK_ID[$action]]];
}
