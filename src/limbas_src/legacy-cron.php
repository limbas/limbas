<?php
/**
 * @deprecated this file will be removed in the future => use cli.php instead
...
$sqlquery = "SELECT * FROM LMB_CRONTAB WHERE ID = $job AND ACTIVE = " . LMB_DBDEF_TRUE;
$rs = lmbdb_exec($db,$sqlquery) or errorhandle(lmbdb_errormsg($db),$sqlquery,$action,__FILE__,__LINE__);
if(!$rs) {$commit = 1;}
if(lmbdb_fetch_row($rs)) {
	$cronvalue = lmbdb_result($rs,"CONFIG");
	$cron_id = lmbdb_result($rs,"ID");
	$kattempl = lmb_strtolower(lmbdb_result($rs,"TYPE"));
