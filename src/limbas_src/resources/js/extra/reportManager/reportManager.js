...
function loadStructure(sender, element,reportid,gtabid,tgtabid,template,settings) {
    ...
    $(sender).after('<div class="text-center" id="loader-' + lid + '"><img src="assets/images/legacy/wait1.gif"></div>');
...
function getSingleListElement(reportid,gtabid,tgtabid,element) {
    ...
    if (element['haschildren']) {
        ...
        return '<a href="#report-'+reportid+'-'+element['ident']+gid+'" class="list-group-item list-group-item-action" data-bs-toggle="collapse" data-load="report-'+reportid+'-'+element['ident']+gid+'" data-reportid="'+reportid+'" data-template="'+element['name']+'" data-gtabid="'+gtabid+'" data-tgtabid="'+tgtabid+'">' +
            getSingleListElementInner(element) +
            '</a>' +
            '<div class="list-group collapse ps-2" id="report-'+reportid+'-'+element['ident']+gid+'"></div>';
    } 
...
function getSingleListElementInner(element) {
    ...
    return '<div class="row cursor-pointer" ' + editparams + ' data-tgtabid="'+element['tgtabid']+'" data-name="'+element['name']+'">' +
        '<div class="col-10"><i class="lmb-icon align-middle me-2 '+icon+'"></i><span class="align-middle">'+element['name']+'</span></div>' +
        '<div class="col-2 text-end"><i class="'+editicon+'" '+addparams+' data-tgtabid="'+element['tgtabid']+'" data-name="'+element['name']+'"></i></div>' +
        '</div>';
}
...
function getGroupList(reportid,gtabid,tgtabid,rootelement) {
    ...
    let output = '<a href="#report-'+reportid+'-'+rootelement['ident']+gid+'" class="list-group-item list-group-item-action '+bg+'" data-bs-toggle="collapse">' +
        getSingleListElementInner(rootelement)+'</a>' +
        '<div class="list-group collapse ps-2" id="report-'+reportid+'-'+rootelement['ident']+gid+'" data-group="'+rootelement['id']+'">';
...
function loadEditTemplate($element) {
    ...
    if (!tinymceChangeLoaded) {
        ...
        tinymce.activeEditor.on('change', activateSaveButton);
...
function addTemplateToGroupModal(e) {
    ...
    $('#new-template-group').html('<option value="' + $(this).data('add') + '"  data-tgtabid="' + $(this).data('tgtabid') + '" selected>' + $(this).data('name') + '</option>');
    $('#template-copy').hide();
