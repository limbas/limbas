...
class ReportMenuRenderBs4 extends ReportMenuRender {
    ...
    protected function printReportPrintFormInternal($action, $report_id, $gtabid, $ID, $listmode, $report_medium, $report_medium_opt, $report_rename, $resolvedTemplateGroupsStr, $resolvedDynamicDataStr) {
        ...
        $html .= $this->pop_submenu2($lang[1787], "reportModalSelectItem(event,null,'{$gtabid}','{$report_id}','{$ID}',2,'{$listmode}',null,null,null, {$resolvedTemplateGroupsStr}, {$resolvedDynamicDataStr})",null,null,null,'btn-report-archive');
    ...
    private function pop_submenu2($val,$zl,$zv=null,$width=null,$imgclass=null,$id=""){
        if (!empty($id)) {
            $id = 'id="'.$id.'"';
        }
        return '<button type="button" '.$id.' class="btn btn-outline-secondary btn-block mb-3" onclick="'.$zl.'";return false;" title="'.$zv.'">'.$val.'</button>';
    }
}
