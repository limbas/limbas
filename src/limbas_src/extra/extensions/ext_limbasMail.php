...
require_once(COREPATH . 'extra/messages/classes/php/olTemplate.class.php');
require_once(COREPATH . 'extra/messages/classes/php/olMailTreeList.class.php');
require_once(COREPATH . 'extra/messages/classes/php/olMail.class.php');
require_once(COREPATH . 'extra/messages/classes/php/olMimeMessage.class.php');
...
class limbasMail extends olMail{
    ...
    function __construct($table_id, $field_id, $record_id, $rtable_id){
        ...
        require_once(COREPATH . 'extra/messages/config.php');
...
//        print_r(imap_fetchheader($this->con, "$uid", FT_UID));
	...
	function del($uid, $mbox=null, $record_id){
		...
		if ($uid){
			if (($ok = $this->delmail($uid, $mbox))){
                ...
                if (set_joins($this->table_id,$verkn)) {
                    if(del_data($this->rtable_id,$record_id,'delete')){
	...
	private function _get($uid, $mbox){
		...
		$attachments = array();
		if (isset($info->parts) && (lmb_count($info->parts) > 1)){
			$partno = 0;
		...
		return array("fromaddr" => $from, "toaddr" => $to, "date" => $date, "subject" => $subject,
			...
			"attachments" => lmb_count($attachments));
	}
