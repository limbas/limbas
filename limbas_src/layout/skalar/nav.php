...
<div id="hiddenframe" style="height:90%;cursor:pointer;<?=$hiddendispl?>" OnClick="return hide_frame();">
    <i class="lmb-icon lmb-icon-aw lmb-caret-right" style="position:absolute;left:3px;top:4px;cursor:pointer;"></i>
</div>
...
foreach($menu as $key1 => $menuType){
	...
	if($menuType){
		foreach ($menuType as $key2 => $firstLevel) {
			...
			echo "<tr><td class=\"lmbMenuHeaderNav\" $onclickSymbol>";

			# Symbol
			if($firstLevel["gicon"]){
                                echo "<i class=\"lmbMenuHeaderImage ".$firstLevel["gicon"]."\" style=\"position:absolute;margin-left:5px;cursor:pointer\"></i>";
			}
				...
				if($firstLevel["child"]){
					foreach ($firstLevel["child"] as $key3 => $element) {
						...
						foreach ($element["child"] as $key4 => $subelement) {
							...
							echo $img . "<a class=\"lmbMenuItemBodyNav\" $onclick>&nbsp;" . $textToDisplay."</a>";
