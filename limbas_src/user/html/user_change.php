...
document.getElementById("ampel").style.visibility='hidden';
        ...
        if(browser_ns5){
                ...
                document.getElementById(NAME).style.left=window.event.clientX + document.body.scrollLeft;
                document.getElementById(NAME).style.top=window.event.clientY + document.body.scrollTop;
        }
...
checkemail = open("main.php?<?=SID?>&action=user_check_email&ID=<?=$ID?>&e_typ=" + typ + "&e_server=" + server + "&e_user=" + user + "&e_pass=" + pass + "" ,"CheckEmail","toolbar=0,location=0,status=0,menubar=0,scrollbars=1,resizable=0,width=650,height=300");
}

function srefresh() {
	link = confirm("<?=$lang[856]?>");
	if(link) {
		document.location.href="main.php?<?=SID?>&action=user_change&ID=<?=$ID?>&sess_refresh=<?=$ID?>";
	}
}
...
echo "<TR class=\"tabBody\"><TD style=\"width:150px;\">$lang[608]</TD><TD><INPUT TYPE=TEXT style=\"width:150px;\" name=\"vorname\" readonly disabled VALUE=\"".$result_user["username"]."\"></TD></TR>";
...
echo "<TR class=\"tabBody\"><TD style=\"width:150px;\">$lang[143]</TD><TD><INPUT TYPE=TEXT style=\"width:150px;\" name=\"name\" $redo VALUE=\"".$result_user["name"]."\"></TD></TR>";
...
/* --- Sprach Liste --------------------------------------------- */
echo "<TR class=\"tabBody\"><TD width=150>$lang[697]</TD><TD><SELECT STYLE=\"width:160px;\" NAME=\"language\" OnChange=\"this.form.lang_change.value='1';\">";
...
# Datumsformat
echo "<TR class=\"tabBody\"><TD width=150>$lang[2578]</TD><TD><SELECT STYLE=\"width:160px;\" name=\"dateformat\"><OPTION VALUE=1 "; if($result_user["dateformat"] == "1"){echo "SELECTED";} echo">deutsch<OPTION VALUE=2 ";  if($result_user["dateformat"] == "2"){echo "SELECTED";} echo">english<OPTION VALUE=3 "; if($result_user["dateformat"] == "3"){echo "SELECTED";} echo">us</SELECT></TD></TR>";
...
foreach ($gtab["table"] as $key => $value){
	if($gtab["typ"][$key] == 6){
		...
		echo "<tr class=\"tabBody\"><td style=\"width:200px;\">".$lang[2518]."</td><td><input type=\"text\" style=\"width:150px;\" name=\"e_setting[$key][full_name]\" value=\"".$result_user["e_setting"][$key]["full_name"]."\"></td></tr>";
...
<TR><TD VALIGN="top">
<TABLE>
<TR><TD><INPUT TYPE="submit" value="<?=$lang[2122]?>"></TD></TR>
</TABLE>
</TD></TR></TABLE>
