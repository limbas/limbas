    ...
    <table class="kanban-default-form" cellpadding="1" cellspacing="0" width="100%">
        ...
        <tr>
            <td><p><?=$gfield[$gtabid]['spelling'][$gfield[$gtabid]["argresult_name"]["STATUS"]]?></p>
                <?php display_dftyp($gresult,$gtabid,$gfield[$gtabid]["argresult_name"]["STATUS"],$ID,1,null,"width:100px;");?></td>
        </tr>
        <tr>
            <td><p><?=$gfield[$gtabid]['spelling'][$gfield[$gtabid]["argresult_name"]["ASSIGNED"]]?></p>
                <?php display_dftyp($gresult,$gtabid,$gfield[$gtabid]["argresult_name"]["ASSIGNED"],$ID,1,null,"width:100px;");?></td>
        </tr>
        <tr>
        ...
        <tr>
            <td colspan="4">
                <input type="button" value="<?=$lang[33]?>" onclick="kanban_card_edit(<?php echo $ID; ?>);">
                <?php if($ID > 0){
                    echo "<input type=\"button\" value=\"details\" onclick=\"lmb_popupDetails('$gtabid','$ID')\">";
                    echo ($LINK[11] && $gtab['delete'][$gtabid]) ? "<input style=\"float:right\" onclick=\"kanban_card_delete('$gtabid','$ID')\" type=\"button\" value=\"$lang[160]\">" : '';
                    echo ($LINK[164] && $gtab['hide'][$gtabid]) ? "<input style=\"float:right\" onclick=\"kanban_card_archive('$gtabid','$ID')\" type=\"button\" value=\"$lang[1257]\">" : '';
                }?>
            </td>
        </tr>
    </table>
