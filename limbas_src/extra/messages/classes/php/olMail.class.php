...
define('DEFAULT_IMAP_OPTIONS', '/novalidate-cert/notls'); /* we do not support SSL/TLS */
...
class olMail {
	...
	function _qpdecode($s){
		/* preform a regular expression using variables and the ungreedy & execution flags */
		return preg_replace('/=\?(.*)\?(.)\?(.*)\?=\s{0,1}.*/Ue', "\$this->_qpdecode_cb('$1', '$2', '$3')", $s);
	}
	...
	function savemail($mbox, $headers, $body){
		...
		if (($ret = imap_append($this->con,
			"{{$this->config['imap_hostname']}}{$mbox}", $headers."\r\n".$body)))
	...
	function listmails($mbox=NULL, $sort_col=NULL, $sort_desc=false,$msgrange = '',$uids = 0,$prepare_for_link = ''){
		...
		/* TODO: reimplement this horrible part, using inline functions (as of php-5.3) */
		if ($sort_col){
			$code = "\$a = ($sort_desc) ? \$arg2 : \$arg1;\n".
				"\$b = ($sort_desc) ? \$arg1 : \$arg2;\n";
			switch($sort_col){
				case 'date':
					$code .= 'return (strtotime($a->date) == strtotime($b->date)) '.
						'? 0 : (strtotime($a->date) > strtotime($b->date));';
					break;
				case 'size':
					$code .= 'return ($a->size == $b->size) ? 0 : ($a->size > $b->size);';
					break;
				case 'from':
					$code .= "\$_util = new olMail(NULL);\n".
						'return strcmp($_util->_qpdecode($a->from), '.
						'$_util->_qpdecode($b->from));';
					break;
				default:
					$code .= "\$_util = new olMail(NULL);\n".
						'return strcmp($_util->_qpdecode($a->subject), '.
						'$_util->_qpdecode($b->subject));';
			}
			usort($result, create_function('$arg1, $arg2', $code));
		}
		...
		$rows='';
                ...
                if (!$prepare_for_link != '')
                {
                    ...
                    foreach ($result as $_idx=>$_o)
                    {
                        if ($prepare_for_link != '')
                        {
                            $rows .= '<div class="lmbContextRow">
                                       <div class="lmbContextItem">
                                        <i border="0" style="height:12px;" class="lmb-icon-cus lmb-rel-add"></i>
                                        <span class="lmbContextItemIcon" onclick="link_search_mail('.urlencode($_o->uid).',\''.$prepare_for_link.'\')" onmouseout="this.style.color=\'black\';" onmouseover="this.style.color=\'blue\';" style="cursor: pointer; color: black;">
