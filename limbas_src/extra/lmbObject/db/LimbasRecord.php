...
*/
		...
		if (isset($result[0]['fresult'][$tid])) {
			...
			if (1 < count($records)) {
				throw new LimbasRecordException('Found ' . count($records) . ' ' . get_class($this). ' with same ID (' . $id . ')!');
			}
...
// 							}
							if (is_array($value)) {
								...
								$this->log('LimbasRecord(' . $this->refId . ').handleRecords objectcount: ' . count($item->attributes[$fieldname]->getObjects()));
							}
						}
					}
		...
		foreach($attrs as $name => $data) {
			...
			//if ((isset($data) && (!is_array($data) && '' !== $data) || 0 < count($data))) {
			{
				...
				if (is_object($data) && 'LimbasRelation' === get_class($data)) {
					$log = $name . ' LimbasRelation(' . $this->refId . ') ' . count($data->getObjects() . ', ' . var_export($data->hasObjects(), true));
					$this->log($log);	
					if ($data->hasObjects()) {
