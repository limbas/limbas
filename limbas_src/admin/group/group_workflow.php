...
<?if($LINK[293]){?><TR class="tabHpoolItemTR"><TD class="tabHpoolItemActive" OnClick="<?=$LINK["link_url"][293]?>" TITLE="<?=$lang[$LINK["desc"][293]]?>"><?echo "<i style=\"float:left;\" class=\"lmb-icon ".$LINK["icon_url"][293]."\"></i>&nbsp;".$lang[$LINK["name"][293]];?></TD></TR><?}?>
...
<TR class="tabHeader"><TD class="tabHeaderItem"><i class="lmb-icon-cus lmb-icon-8 lmb-workflow-car"></i>&nbsp;<b><?=$lang[2035]?></b></TD><TD class="tabHeaderItem"><?=$lang[2514]?></TD><TD class="tabHeaderItem"><?=$lang[2513]?></TD></TR>
<?
if($rulelist_){
	foreach ($rulelist_['id'] as $key => $rid){
		...
		echo "<TR><TD width=\"200\">&nbsp;&nbsp;&nbsp;&nbsp;".$rulelist_["name"][$key]."</TD><TD>";
		if($levelrule["hasview"][$rid] OR !$group_level){
			echo "<INPUT TYPE=\"CHECKBOX\" NAME=\"setrule[".$rid."]\" $CHECKED1>";
		}else{
			echo "<INPUT TYPE=\"checkbox\" $CHECKED1 readonly disabled style=\"opacity:0.3;filter:Alpha(opacity=30);\">";
		}
		echo "</TD><TD>";
		if($CHECKED1){echo "<INPUT TYPE=\"CHECKBOX\" NAME=\"sethidden[".$rid."]\" $CHECKED2>";}
		echo "</TD></TR>";
	}
}
...
<TR><TD colspan="3"><INPUT TYPE="submit" VALUE="<?=$lang[2283]?>" NAME="change_rules">&nbsp;&nbsp;&nbsp;&nbsp;<?=$lang[2107]?>:<input type="checkbox" name="addsubgroup" value="1"></TD></TR>
<?}?>
