	...
	foreach($link_groupdesc as $key => $value){
		...
		echo "<TR class=\"tabHeader\"><TD class=\"tabHeaderItem\" COLSPAN=\"14\">".$link_groupdesc[$key][0]."</TD></TR>";
                ...
                <TR class="tabHeader">
                    ...
                    <TD class="tabHeaderItem" nowrap>&nbsp;<?=$lang[4]?> &nbsp;</TD>
                    <TD class="tabHeaderItem" nowrap>&nbsp;<?=$lang[126]?> &nbsp;</TD>
                    <TD class="tabHeaderItem" nowrap>&nbsp;<A HREF="Javascript:document.form1.order.value='ACTION';document.form1.submit();"><?=$lang[544]?></A> &nbsp;</TD>
		...
		foreach($result_links["sort"] as $bzm => $value0){
			...
			if($result_links["local"][$bzm] == 1){$bg = "red";$color = "";}elseif($result_links["local"][$bzm] == 2){$bg = "green";$color = "white";}else{$bg = "";$color = "";}
			?>
				<TR class="tabBody">
	            ...
	            <TD nowrap VALIGN="TOP"><INPUT TYPE="TEXT" SIZE="15" VALUE="<?= htmlentities($result_links["link_name"][$bzm],ENT_QUOTES,$GLOBALS["umgvar"]["charset"]) ?>" NAME="link_name[<?=$result_links["id"][$bzm]?>]" onchange="document.form1.change_link_<?=$result_links["id"][$bzm]?>.value='1'"></TD>
	            <TD nowrap VALIGN="TOP"><INPUT TYPE="TEXT" SIZE="25" VALUE="<?= htmlentities($result_links["beschreibung"][$bzm],ENT_QUOTES,$GLOBALS["umgvar"]["charset"]) ?>" NAME="link_desc[<?=$result_links["id"][$bzm]?>]" onchange="document.form1.change_link_<?=$result_links["id"][$bzm]?>.value='1'"></TD>
				<TD nowrap VALIGN="TOP"><INPUT TYPE="TEXT" SIZE="15" VALUE="<?= htmlentities($result_links["action"][$bzm],ENT_QUOTES,$GLOBALS["umgvar"]["charset"]) ?>" NAME="link_action[<?=$result_links["id"][$bzm]?>]" onchange="document.form1.change_link_<?=$result_links["id"][$bzm]?>.value='1'"></TD>
				<TD nowrap VALIGN="TOP"><INPUT TYPE="TEXT" SIZE="40" VALUE="<?= htmlentities($result_links["link_url"][$bzm],ENT_QUOTES,$GLOBALS["umgvar"]["charset"]) ?>" NAME="link_url[<?=$result_links["id"][$bzm]?>]" onchange="document.form1.change_link_<?=$result_links["id"][$bzm]?>.value='1'"></TD>
				<TD nowrap VALIGN="TOP"><INPUT TYPE="TEXT" SIZE="30" VALUE="<?= htmlentities($result_links["icon_url"][$bzm],ENT_QUOTES,$GLOBALS["umgvar"]["charset"]) ?>" NAME="link_iconurl[<?=$result_links["id"][$bzm]?>]" onchange="document.form1.change_link_<?=$result_links["id"][$bzm]?>.value='1'"></TD>
	            <TD nowrap VALIGN="TOP"><INPUT TYPE="TEXT" SIZE="30" VALUE="<?= htmlentities($result_links["help_url"][$bzm],ENT_QUOTES,$GLOBALS["umgvar"]["charset"]) ?>" NAME="link_helpurl[<?=$result_links["id"][$bzm]?>]" onchange="document.form1.change_link_<?=$result_links["id"][$bzm]?>.value='1'"></TD>
			    ...
			    <TD nowrap ALIGN="CENTER">&nbsp;&nbsp;&nbsp;<?php if($result_links["icon_url"][$bzm]){echo "<i class=\"lmb-icon ".$result_links["icon_url"][$bzm]."\"></i>";}?></TD>
                            <TD nowrap ALIGN="CENTER" VALIGN="CENTER"><?php if($supervisor OR $result_links["local"][$bzm]){?><A HREF="main_admin.php?action=setup_links&del=1&id=<?= $result_links["id"][$bzm] ?>"><i class="lmb-icon lmb-trash" BORDER="0"></i></A><?php }?></TD>
...
<TR><TD ALIGN="LEFT" COLSPAN="14"><INPUT TYPE="submit" VALUE="<?=$lang[522]?>" NAME="change"></TD></TR>
<TR><TD COLSPAN="14"><HR></TD></TR>
...
<TD></TD>
<TD COLSPAN="2" valign="top"><INPUT TYPE="submit" VALUE="<?=$lang[540]?>" NAME="add"></TD>
</TR>
