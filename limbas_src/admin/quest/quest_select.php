...
while(odbc_fetch_row($rs, $bzm)) {
        echo "<TR><TD><A HREF=\"main_admin.php?".SID."&action=setup_quest_sql&quest_id=".odbc_result($rs, "ID")."\">".odbc_result($rs, "QUEST_NAME")."</A></TD>";
        echo "<TD>".$userdat[vorname][odbc_result($rs, "ERSTUSER")]." ".$userdat[name][odbc_result($rs, "ERSTUSER")]."</TD>";
        echo "<TD <TD ALIGN=\"CENTER\"><A HREF=\"main_admin.php?".SID."&action=setup_quest_select&del=1&quest_id=".odbc_result($rs, "ID")."\"><i class=\"lmb-icon lmb-trash\" BORDER=\"0\"></i></A></TD></TR>";
$bzm++;
}
