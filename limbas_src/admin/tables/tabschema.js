...
function drag(evt) {
	if (el1 != null) {
		...
		if(ityp == 1){
			...
			cl = ax - px + dx;
			ct = ay - py + dy;
			if(cl > 0){el1.style.left = cl;}
			if(ct > 0){el1.style.top = ct;}
		// resize
		}else if(ityp == 2){
			...
			el1.style.width = ax - px + ex;
			el1.style.height = ay - py + ey;
			
			
			el2.style.width = el1.style.offsetWidth;
			el2.style.height = ay - py + ey - 50;
		}
	}
	//paint_lines();
	return false;
...
function sendkeydown(evt) {
	if(evt.keyCode == 13 && !evt.shiftKey && !stopEvent){
		setDrag();
...
function js_line(tabid,vtabid,fieldid,fel,tel,bzm,dtp){
	...
	if(browser_ns5){
		var pos_from_x = from_el.offsetParent.offsetParent.offsetLeft + from_el.offsetLeft + tab_el.offsetWidth + 20;
		...
		var pos_to_y = to_el.offsetParent.offsetParent.offsetTop + to_el.offsetTop + to_el.offsetParent.offsetTop - pos_from_y - to_el.parentNode.scrollTop + 8;
	}else{
		var pos_from_x = from_el.offsetParent.offsetParent.offsetParent.offsetLeft + from_el.offsetLeft + tab_el.offsetWidth + 20;
		var pos_from_y = from_el.offsetParent.offsetParent.offsetParent.offsetTop + from_el.offsetParent.offsetParent.offsetTop + from_el.offsetParent.offsetTop + from_el.offsetTop - from_el.parentNode.scrollTop + 8;
		var pos_to_x = to_el.offsetParent.offsetParent.offsetParent.offsetLeft + to_el.offsetLeft - pos_from_x - 25;
		var pos_to_y = to_el.offsetParent.offsetParent.offsetParent.offsetTop + to_el.offsetParent.offsetParent.offsetTop + to_el.offsetParent.offsetTop + to_el.offsetTop - pos_from_y - to_el.parentNode.scrollTop + 10;
	}
	...
	// oberes Limit
	//alert((to_el.offsetParent.offsetParent.offsetTop + to_el.offsetTop + to_el.offsetParent.offsetTop - from_el.parentNode.scrollTop)+' '+to_el.offsetParent.offsetParent.offsetTop)
	

	kont_el.style.left = pos_from_x;
	kont_el.style.top = pos_from_y;
	
	eval(jg+".clear();");
	eval(jg+".setStroke(1);");
	eval(jg+".setColor('#000000');");
	eval(jg+".drawLine(1, 1, "+pos_to_x+", "+pos_to_y+");");
	eval(jg+".setFont('arial','12px');");
	if(fieldid){
		var strg = "<img src=\"pic/center.gif\" STYLE=\"cursor:help;\" OnClick=\"lmbAjax_fieldinfo(event,2,'"+tabid+"','"+fieldid+"','"+vtabid+"');\">";
		eval(jg+".drawString(strg,-15,-2);");
		var strg = "<img src=\"pic/right.gif\" STYLE=\"cursor:help;\" OnClick=\"lmbAjax_fieldinfo(event,2,'"+tabid+"','"+fieldid+"','"+vtabid+"');\">";
		eval(jg+".drawString(strg,"+(pos_to_x+2)+", "+(pos_to_y-1)+");");
	}else{
		var strg = "<img src=\"pic/center.gif\" STYLE=\"cursor:help;\" OnClick=\"lmbAjax_fieldinfo(event,3,document.form1.viewid.value,'"+fel+"','"+tel+"','');\">";
		eval(jg+".drawString(strg,-15,-2);");
		var strg = "<img src=\"pic/right.gif\" STYLE=\"cursor:help;\" OnClick=\"lmbAjax_fieldinfo(event,3,document.form1.viewid.value,'"+fel+"','"+tel+"','');\">";
		eval(jg+".drawString(strg,"+(pos_to_x+2)+", "+(pos_to_y-1)+");");
	}
	eval(jg+".paint();");
